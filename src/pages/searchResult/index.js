...
const Index = () => {
  ...
  useEffect(() => {
    ...
    if (storedFormData) {
      ...
      axios.post('http://localhost:8001/api/fligth/filter', parsedFormData)
        .then(response => {
      ...
      {responseData.map((data, index) => (
        <div key={index} className="bg-[#D9D9D9] rounded-lg shadow-md p-6 flex flex-col justify-between items-center">
          ...
          <div className="mt-4 text-center">
            <div>Date Depart: {new Date(data.date_depart).toISOString().split('T')[0]}{" "}
          Date Return: {new Date(data.date_return).toISOString().split('T')[0]}</div>
          </div>
        </div>
      ))}
    </div>
